...
if __name__ == '__main__':
    ...
    u, v = cp.fill({k: u(1000, true_theta) for k, u in cp.draw_u.items()}, {}, true_theta, cp.draw_u.keys())
    x = v['X']
    o = {'V1': v['V1']}
    y = v['Y']
    ...
    _, est_v = cp.fill({k: u(100000, theta) for k, u in cp.draw_u.items()}, {}, true_theta, cp.draw_u.keys())
    for rv in ['X', 'Y', 'V1']:
